...
def get_user_id(username):
    ...
    if response.status_code == 429:
        print(f"⚠️ Rate limited on user lookup for {username}")
        return None
...
def get_latest_post(user_id):
    ...
    if response.status_code == 429:
        print("⚠️ Rate limited on tweets fetch!")
        return None
    response.raise_for_status()
    ...
    if "data" in data and len(data["data"]) > 0:
        tweet = data["data"][0]
        image_url = ""
        # Placeholder: if you want media URLs, a separate media API call is needed
        return {
...
def main():
    ...
    try:
        ...
        if not user_id:
            print(f"⚠️ Skipping {username} due to rate limit / missing ID")
            return

        time.sleep(1)  # polite delay
        post = get_latest_post(user_id)
        if not post:
            print(f"⚠️ No post found for {username}")
            return
